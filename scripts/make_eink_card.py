...
def main() -> int:
    ...
    ap.add_argument("--list", action="store_true",
                    help="print available targets, moods, periods and exit")
    ...
    ap.add_argument("--tag", action="append", default=[], dest="any_tags",
    ...
    ap.add_argument("--exclude-filter", action="append", default=[],
    ...
    ap.add_argument("--sort", default="fit",
                    choices=["fit", "quality", "year", "artist", "title",
                             "random", "as-filtered"])
    ap.add_argument("--seed", type=int, default=42)
    ap.add_argument("--target", default="gooddisplay-315-diy",
                    choices=sorted(TARGETS))
    ap.add_argument("--fit", choices=["contain", "cover", "stretch"])
    ap.add_argument("--dither", default="floyd-steinberg",
                    choices=["none", "floyd-steinberg", "atkinson"])
    ap.add_argument("--slow-dither", action="store_true",
    ...
    ap.add_argument("--overwrite", action="store_true",
                    help="replace files a previous export wrote")
    a = ap.parse_args()
    ...
    spec = PlaylistSpec(
        artists=a.artist, genres=a.genre, moods=a.mood, periods=a.period,
        year_from=a.year_from, year_to=a.year_to,
        any_tags=a.any_tags, all_tags=a.all_tags, not_tags=a.not_tags,
        exclude_filters=a.exclude_filter,
        min_fit=a.min_fit, min_quality=a.min_quality,
        require_dossier=a.dossier_only, limit=a.limit, sort=a.sort, seed=a.seed,
    )

    sidecars = list(iter_sidecars())
    dossier_ids = {
        wid for wid, sc in sidecars
        if isinstance(sc.get("dossier"), dict) and sc["dossier"]
    }
    res = build(sidecars, spec, ratings=load_ratings(RATINGS),
                dossier_ids=dossier_ids)

    print(f"candidates {res.total_candidates}  matched {res.matched}"
          f"  selected {len(res.work_ids)}")
    if res.coverage["excluded_for_missing_metadata"]:
        print("  dropped for missing metadata: "
              + ", ".join(f"{k}={v}" for k, v in
                          res.coverage["excluded_for_missing_metadata"].items()))
    if res.facets["genre"]:
    ...
    rep = export(
        items, a.out, target, master_for=master_for, fmt=a.format,
        method=a.dither, fit=a.fit,
        compress_range=not a.no_range_compress,
        fast=not a.slow_dither,
        overwrite=a.overwrite, dry_run=not a.write,
        spec=spec.__dict__, progress=progress if a.write else None,
    )
    d = rep.as_dict()
    print(f"\n{'WROTE' if a.write else 'WOULD WRITE'} {d['written']} file(s)"
          + (f", {d['bytes_written'] / 1e6:.1f} MB" if a.write else ""))
    if d["skipped"]:
        print(f"  skipped {d['skipped']}: " + "; ".join(
            f"{w} ({why})" for w, why in d["skipped_detail"][:6]))
    if not a.write:
