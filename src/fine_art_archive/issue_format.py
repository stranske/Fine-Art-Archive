"""Validate an issue body against docs/AGENT_ISSUE_FORMAT.md.

Why this exists: the automated lanes only ever see an issue through a *label*.
An issue filed with no label and no Tasks/Acceptance block is invisible to the
whole pipeline — nothing validates it, nothing optimises it, nothing claims it.
That is how four audit findings (#406-409) sat unactionable: they are good
diagnostics filed by `automation_audit.py`, but they are not work orders.

So this module is the single definition of "agent-processable", used by both
ends:

  * the repo-side guard workflow, which validates every issue on open/edit and
    applies `agents:format` when it fails — which in turn triggers the existing
    issue-optimizer's format phase, so the fix is routed rather than merely
    reported;
  * any local script that files issues, which can pre-flight with
    `python -m fine_art_archive.issue_format <file>` and refuse to file junk.

The rules mirror the format guide rather than inventing a parallel standard:
Tasks and Acceptance Criteria are REQUIRED; Why / Scope / Non-Goals are
recommended; and at least one acceptance criterion must name a real test,
runnable command, or observable verification gate (guide section 2).
"""
