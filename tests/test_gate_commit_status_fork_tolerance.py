...
red Gate, and the true verdict was never printed anywhere.

The guard is deliberately narrow.  A 403 on a *same-repo* pull request is a
real permission regression and must still fail the job, so this test pins both
directions.  It runs the step's actual JavaScript (extracted from the workflow
file) under Node against stubbed ``github``/``context``/``core`` objects, so it
fails if the tolerance is removed or widened.
"""
...
RUNNER_JS = textwrap.dedent(
    ...
    async function runCase({ headRepo, baseRepo, error, state }) {
      ...
      const sandbox = {
        ...
        context: {
          repo: { owner: 'stranske', repo: 'Fine-Art-Archive' },
          sha: 'basesha',
          payload: {
            pull_request: {
    ...
    const FORK = {
      headRepo: 'outside-contributor/Fine-Art-Archive',
      baseRepo: 'stranske/Fine-Art-Archive',
    };
    const SAME = {
      headRepo: 'stranske/Fine-Art-Archive',
      baseRepo: 'stranske/Fine-Art-Archive',
    };
...
def test_same_repo_403_still_fails_the_gate(outcomes: dict[str, Any]) -> None:
    """A permission regression on a same-repo PR must stay loud."""
